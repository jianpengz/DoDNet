...
class MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4):
        ...
        inx = 16
        self.offset_normalizer = torch.tensor([[inx * (2 ** (i - 1)), inx * (2 ** (i - 1)), inx * (2 ** (i - 1))] for i in
                                          range(self.n_levels, 0, -1)]).cuda()
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        attention_weights = self.attention_weights(query).view(N, Len_q, self.n_heads, self.n_levels * self.n_points) # v0
        attention_weights = F.softmax(attention_weights, -1).view(N, Len_q, self.n_heads, self.n_levels, self.n_points)
        if reference_points.shape[-1] == 3:
            sampling_locations = reference_points[:, :, None, :, None, :] \
                                 + sampling_offsets / (self.offset_normalizer[None, None, None, :, None, :])
